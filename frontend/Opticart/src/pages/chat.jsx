...
function Chat() {
  // 1. STATE: History of the conversation
  ...
  // 2. STATE: Input box text
  const [input, setInput] = useState("");
  
  // 3. STATE: Loading status (disables button while waiting)
  ...
  // 4. FUNCTION: Send message to Python Backend
  const sendMessage = async () => {
    if (!input.trim()) return;

    // A. Add User's Message to UI immediately
    ...
    try {
      ...
      // D. Create a "Recipe Card" message
      // We check if the data has 'dish_name' to confirm it's a recipe
      if (data.dish_name) {
          const aiResponse = { 
            sender: 'ai', 
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-3xl p-5 rounded-2xl shadow-sm ${
              msg.sender === 'user' 
                ? 'bg-temporary-turqoise text-white' // Uses your custom color
